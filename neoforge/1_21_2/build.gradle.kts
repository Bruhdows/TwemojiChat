...
extensions.configure<NeoForgeExtension> {
    setVersion("21.2.1-beta")

    runs {
        create("client") {
            client()
            gameDirectory.set(project.layout.projectDirectory.dir("run/neoforge/1_21_2"))
        }
...
tasks.processResources {
    filesMatching("META-INF/neoforge.mods.toml") {
        expand(
            mapOf(
                "minecraft_version" to "1.21.2",
                "minecraft_version_range" to "[1.21.2,1.22)",
                "neo_version" to "21.2.1-beta",
                "neo_version_range" to "[21.2,)",
