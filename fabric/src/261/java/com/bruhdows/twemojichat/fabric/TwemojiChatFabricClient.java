...
public final class TwemojiChatFabricClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientReceiveMessageEvents.ALLOW_CHAT.register((message, signedMessage, sender, params, receptionTimestamp) -> {
            Minecraft.getInstance().gui.getChat().addPlayerMessage(RUNTIME.rewriteReceivedMessage(message), signedMessage.signature(), null);
            return false;
        });
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            RUNTIME.onScreenInit(screen);
            ScreenEvents.afterExtract(screen).register((currentScreen, guiGraphicsExtractor, mouseX, mouseY, tickDelta) -> RUNTIME.onScreenRender(currentScreen));
